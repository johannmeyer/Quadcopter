...
void process_mode(uint8_t mode)
{
    ...
    case 2:									// Manual mode
		...
		case 3:									// Calibration mode
			if ((prev_mode == SAFE_MODE || prev_mode == CALIBRATION_MODE) && !isCalibrated())
			{
...
int main(void)
{
  ...
  logCore = (core *) malloc(sizeof(core));
		...
		if (rx_queue.count >= sizeof(packet))
    {
      ...
      if(isCalibrated())
      {
        printf("%10ld | m%d | %x %x %x %x %x | ", get_time_us(), prev_mode, logCore->header.mode, logCore->body.roll, logCore->body.pitch, logCore->body.yaw, logCore->body.lift);
      ...
      {
        //write_log_entry(get_time_us(), prev_mode, logCore, ae, phi, theta, psi, sp, sq, sr, sax, say, saz, bat_volt, temperature, pressure);
